...
public class UIPage_EventMenu : A_UIPage
{
    enum DialogType
    {
        Dialog_BoxEventMenu,
    ...
    protected override async UniTask OnAwake()
    {
        ...
        {
            { DialogType.Dialog_BoxEventMenu, null},
        };
        await base.OnAwake();
    }
    public override async UniTask InitializationAsync()
    {
        dislogs_Show_Parameters = (inputParameters[0] as Dictionary<string, Func<UniTask>>, 0);
        await ChangeDialogAsync(DialogType.Dialog_BoxEventMenu.ToString(), dislogs_Show_Parameters.main);
    }
